...
const VisitedAppointment = () => {
    ...
    useEffect(() => {
        setLoading(true)
        axios.get(`https://drab-blue-mite-belt.cyclic.app/appointment?token=${localStorage.getItem('dqDoctorAu')}&status=visited`)
        .then(res => {
            setData(res.data)
            setLoading(false)
        })
      ...
      if(loading){
        ...
        return (
          <h2 style={{ textAlign: "center", color: "red",fontSize:'22px' }}>
            Sorry! There are no patients who have visited yet.
          </h2>
        );
      }
